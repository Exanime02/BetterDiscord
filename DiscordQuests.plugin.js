 ...
 * @description Complete quests with a shortcut. Default: Ctrl+Q.
 * @version 4.0.0
...
module.exports = class DiscordQuests {
    constructor() {
        this.defaultSettings = { shortcutKey: "q", modifier: "ctrl" };
    ...
    handleKeyDown(event) {
        ...
        switch (modifier) {
            case "ctrl":    ctrlMatch = hasCtrl;  shiftMatch = !hasShift; altMatch = !hasAlt; break;
            case "shift":   ctrlMatch = !hasCtrl; shiftMatch = hasShift;  altMatch = !hasAlt; break;
            case "alt":     ctrlMatch = !hasCtrl; shiftMatch = !hasShift; altMatch = hasAlt;  break;
            case "ctrl+shift": ctrlMatch = hasCtrl; shiftMatch = hasShift; altMatch = !hasAlt; break;
            case "ctrl+alt":   ctrlMatch = hasCtrl; shiftMatch = !hasShift; altMatch = hasAlt; break;
            case "shift+alt":  ctrlMatch = !hasCtrl; shiftMatch = hasShift; altMatch = hasAlt; break;
            case "ctrl+shift+alt": ctrlMatch = hasCtrl; shiftMatch = hasShift; altMatch = hasAlt; break;
            default: ctrlMatch = hasCtrl; shiftMatch = !hasShift; altMatch = !hasAlt;
        }
    ...
    async checkForUpdates(showUpToDateToast = true) {
        ...
        try {
            if (!this.UPDATE_URL) { toast("No update URL configured.", "info"); return; }
            ...
            if (!remoteVersion) { toast("Update check error: could not read remote version.", "error"); return; }
            const cmp = compareVersions(remoteVersion, this.CURRENT_VERSION);
            if (cmp <= 0) {
                if (showUpToDateToast) toast(`You are up to date (v${this.CURRENT_VERSION}).`, "success");
                return;
            }
            ...
            try {
                if (BdApi?.UI?.showToast) BdApi.UI.showToast(`Update check error: ${String(err.message || err)}`, { type: "error" });
            } catch (e) {}
        }
    }
    ...
    runSnippet() {
        try {
            console.log("[DiscordQuests] Shortcut triggered - Running...");
            ...
            if (quests.length === 0) {
                console.log("[DiscordQuests] You have no incomplete quests!");
            } else {
                let doJob = function () {
                    ...
                    if (taskName === "WATCH_VIDEO" || taskName === "WATCH_VIDEO_ON_MOBILE") {
                        ...
                        if (!isApp) {
                            console.log(`[DiscordQuests] This no longer works in the browser for non-video quests. Use the Discord desktop app to complete quest ${questName}!`);
                        } else {
    ...
    getSettingsPanel() {
        ...
        style.textContent = `
            ...
            .dq-label { width: 220px; font-weight:600; font-size:16px; color:var(--text-normal,#FFFFFF); }
            .dq-input, .dq-select { width: 200px; }
            .dq-input input, .dq-select select { width: 100%; padding: 10px 12px; border-radius: 8px; background: var(--background-modifier-hover,#2f3136); border: 1px solid rgba(0,0,0,0.2); color: var(--text-normal,#FFFFFF); box-sizing: border-box; font-size: 14px; }
            ...
            .dq-update-title { font-weight: 600; font-size: 16px; margin-bottom: 6px; color: var(--text-normal,#FFFFFF); }
        ...
        inputEl.addEventListener("input", (e) => { this.settings.shortcutKey = e.target.value; saveSettings(); });
        ...
        noteKey.textContent = "Key for the shortcut (e.g.: q, x, F1).";
        ...
        updateNote.textContent = "Check and download the latest available version.";
        ...
        updateBtn.addEventListener("click", () => { this.checkForUpdates(true); });
